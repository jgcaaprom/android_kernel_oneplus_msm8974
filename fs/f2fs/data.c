...
static ssize_t check_direct_IO(struct inode *inode, int rw,
		...
		const struct iov_iter *iter, loff_t offset, unsigned long nr_segs)
#else
...
#endif
{
	...
	struct inode *inode = mapping->host;
#ifdef CONFIG_AIO_OPTIMIZATION
	size_t count = iov_iter_count(iter, nr_segs);
#else
...
#ifdef CONFIG_AIO_OPTIMIZATION
	if (check_direct_IO(inode, rw, iter, offset, nr_segs))
		return 0;
#else
...
#ifdef CONFIG_AIO_OPTIMIZATION
	err = check_direct_IO(inode, rw, iter, offset, nr_segs);
#else
...
#ifdef CONFIG_AIO_OPTIMIZATION
	err = blockdev_direct_IO(rw, iocb, inode, iter, offset, nr_segs,
							get_data_block_dio);
#else
